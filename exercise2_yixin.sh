...
for file in `ls ./fluxes`; do
	...
	awk ' { sum[$2]+=$4; n[$2]+=1 } END {for (i in sum) print sum[i]/n[i]} '\
	     ./fluxes/$file >./monthly_prec/$filename
done
...
for i in `seq 1 12`; do
	paste ./temp_files/month$i.txt ./temp_files/lat.txt \
		| awk ' {sum+=$1*cos($2/180*3.14159); weight_sum+=cos($2/180*3.14159)} \
                 END {print sum/weight_sum}' >>result.txt   # write the final result to result.txt
done
